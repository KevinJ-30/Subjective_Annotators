...
class MDAgreementDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Defensive cast: EPIC adapter should already provide int original_id,
        # but this prevents hard crashes if something slips through.
        try:
            text_id_int = int(text_id)
        except Exception:
            # last resort: stable hash to int range (should not happen in normal use)
            text_id_int = abs(hash(str(text_id))) % (2**31)
        ...
        return {
            ...
            "text_id": torch.tensor(text_id_int, device=self.device),
        }
...
def subsample_annotators(data, n_annotators, random_state=42):
    ...
    if n_annotators >= len(unique_annotators):
        print(
            f"Requested {n_annotators} annotators but only {len(unique_annotators)} available. Using all annotators."
        )
        return data, unique_annotators.tolist()

    np.random.seed(random_state)
    selected_annotators = np.random.choice(
        unique_annotators, n_annotators, replace=False
    )
