...
class AnnotatorGrouper:
    ...
    def fit_transform(self, data):
        ...
        for annotator in unique_annotators:
            ...
            if mask.sum() > 0:
                # For multiclass, use one-hot encoding of labels
                label_counts = data[mask]['answer_label'].value_counts(normalize=True)
                profiles[annotator] = label_counts.reindex(range(5)).fillna(0).values
        
        # Convert to array for clustering
        annotator_profiles = np.array(list(profiles.values()))
    ...
    def transform(self, data):
        ...
        Parameters:
        data: pandas DataFrame with columns ['annotator_id', 'answer_label', ...]
        """
        ...
        for group_id, annotators in self.group_mappings.items():
            ...
            majority_votes = group_data.groupby(group_data.index)['answer_label'].agg(
                lambda x: pd.Series.mode(x)[0] if not x.empty else -1
            )
            
            # Update labels and group IDs
            transformed_data.loc[group_mask, 'answer_label'] = transformed_data.index.map(majority_votes)
